...
def save(image,boxes,classes,scores,index):
    
    box=()
    for i in range(min(20, boxes.shape[0])):
        if scores is None or scores[i] > 0.98:
            ...
            x = round(xmin*img_width)
            y = round(ymin*img_height)
            width = round(xmax*img_width)
            height = round(ymax*img_height)
            print(x,y,width,height)

            outfile1 = "non"+str(index)+".jpg"
         
            print(outfile1)
            region=im.crop((x, y, width+10, height+10))
  ...
  with tf.Session(graph=detection_graph, config=config) as sess:
    ...
    while frame_num:
      ...
      save(image,
           ...
           np.squeeze(scores),index=ultimate_index) 
      ultimate_index=ultimate_index+1
